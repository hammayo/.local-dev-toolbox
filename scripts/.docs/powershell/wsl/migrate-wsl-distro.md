...
| `--backup-path`     | `C:\Backups\WSL` (or `WSL_BACKUP_PATH` env)  | Directory for exported tar              |
| `--install-path`    | `D:\WSL\<target>` (or `WSL_INSTALL_ROOT` env)| Install directory for new distro        |
| `--default-user`    | Current `$env:USERNAME` (or `WSL_DEFAULT_USER` env) | Default user in `/etc/wsl.conf`  |
