...
2. Open `_sandbox-config.wsb` and update the `<HostFolder>` path to point to the `scripts/powershell/sandbox` directory on **your** machine.
3. Set Chocolatey source credentials as environment variables on the host **before** launching (the sandbox inherits them via the mapped folder at runtime — do not commit credentials to the script):
   ```powershell
   $env:CHOCO_SOURCE_URL      = "https://nexus.example.com/repository/nuget-chocolatey/"
   $env:CHOCO_SOURCE_USER     = "your-username"
   $env:CHOCO_SOURCE_PASSWORD = "your-password"
   ```
4. Double-click `_sandbox-config.wsb` to launch a new sandbox.
5. The setup script starts automatically and prompts **"Press ENTER to continue or ESC to quit"**.
6. Press Enter — the pipeline installs everything in sequence.
7. Monitor progress in the console. Logs are written to `C:\TEMP\sandbox_setup.log`.
...
| MappedFolder | Host sandbox dir → `C:\TEMP` | Makes all scripts available inside the sandbox |
| LogonCommand | `setup-wsb.ps1` (elevated) | Auto-runs the setup pipeline on sandbox start |

> **Before first use:** Edit `<HostFolder>` in this file to the absolute path of the `scripts/powershell/sandbox` directory on your machine. The XML format does not support variables.
...
All configuration is centralised in `setup-wsb.ps1`'s `$Script:Config` hashtable. Values are resolved from environment variables first, then fall back to the defaults below. **Never commit credentials — supply them via environment variables.**

| Key | Env var | Default | Description |
|-----|---------|---------|-------------|
| `LogFile` | `SANDBOX_LOG_FILE` | `C:\TEMP\sandbox_setup.log` | Path to the log file |
| `DownloadsFolder` | `SANDBOX_DOWNLOADS` | `C:\TEMP\Downloads` | Working directory for downloaded installers |
...
| `NodeVersion` | `NODE_VERSION` | `14.21.3` | Pinned Node.js version |
| `AngularVersion` | `ANGULAR_VERSION` | `11.0.6` | Pinned Angular CLI version |
| `ForcourtVersion` | `FORCOURT_VERSION` | `2.6.1.1` | Pinned Forecourt Service version |
| `ChocoSource.Name` | `CHOCO_SOURCE_NAME` | `choco-dev` | Chocolatey source name |
| `ChocoSource.Url` | `CHOCO_SOURCE_URL` | `""` | NuGet feed URL |
| `ChocoSource.User` | `CHOCO_SOURCE_USER` | `""` | Feed username |
| `ChocoSource.Password` | `CHOCO_SOURCE_PASSWORD` | `""` | Feed password — **env var only, never hardcode** |
