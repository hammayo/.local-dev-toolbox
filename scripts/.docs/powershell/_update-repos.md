...
Two variables control which directories are scanned. They can be set via CLI argument, environment variable, or by editing the defaults in the script directly.

| Variable | CLI arg | Env var | Default |
|----------|---------|---------|---------|
| `$Script:DefaultRootPath` | `--root-path` | `UPDATE_REPOS_ROOT` | `D:\Repos` |
| `$Script:ChildFolderPrefix` | `--prefix` | `UPDATE_REPOS_PREFIX` | `Hydra` |
