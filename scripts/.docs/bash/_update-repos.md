...
Two variables at the top of the script control which directories are scanned. They can be set via CLI argument, environment variable, or by editing the defaults in the script directly.

| Variable | CLI arg | Env var | Default |
|----------|---------|---------|---------|
| `DEFAULT_ROOT_PATH` | `--root-path` | `UPDATE_REPOS_ROOT` | `$HOME/repos` |
| `CHILD_FOLDER_PREFIX` | `--prefix` | `UPDATE_REPOS_PREFIX` | `Hydra` |

Use an empty string for `--prefix` (or `UPDATE_REPOS_PREFIX=''`) to match all child folders.
...
| `--prefix <prefix>`   |                      | Child folder prefix to match as repos (default: `Hydra`, `''` = all) |
