param (
    # Path to the Git repository. Defaults to the current directory, or the
    # GIT_HISTORY_REPO_PATH environment variable if set.
    [Parameter(Mandatory = $false)]
    [string]$RepositoryPath = $(
        if ($env:GIT_HISTORY_REPO_PATH) { $env:GIT_HISTORY_REPO_PATH }
    ...
    # Base URL for ticket hyperlinks in release notes, e.g.
    # "https://dev.azure.com/myorg/myproject/_workitems/edit/"
    # Falls back to the GIT_HISTORY_TICKET_URL env var, then no links.
    [Parameter(Mandatory = $false)]
    [string]$TicketBaseUrl = $(
        if ($env:GIT_HISTORY_TICKET_URL) { $env:GIT_HISTORY_TICKET_URL }
        else { "" }
    ),
...
try 
{
    ...
    if ($tickets.Count -gt 0) {
        ...
        foreach ($ticket in $tickets) {
            ...
            if ($TicketBaseUrl) {
                $releaseNotes += "- [$ticketNumber]($TicketBaseUrl$ticketNumber)`n"
            } else {
                $releaseNotes += "- #$ticketNumber`n"
            }
        }
        $releaseNotes += "`n"
    }
