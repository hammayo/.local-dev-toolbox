...
# Override via --root-path argument or UPDATE_REPOS_ROOT env var.
$Script:DefaultRootPath = if ($env:UPDATE_REPOS_ROOT) { $env:UPDATE_REPOS_ROOT } else { 'D:\Repos' }

# Prefix for immediate child folders to treat as repositories.
# Override via --prefix argument or UPDATE_REPOS_PREFIX env var.
# Use '' (empty string) to match all child folders.
$Script:DefaultChildFolderPrefix = if ($env:UPDATE_REPOS_PREFIX -ne $null) { $env:UPDATE_REPOS_PREFIX } else { 'Hydra' }
$Script:ChildFolderPrefix = $Script:DefaultChildFolderPrefix
# -------------------------------------------------------------------------
...
{
    $result = [ordered]@{
        RootPath = $Script:DefaultRootPath; Prefix = $Script:DefaultChildFolderPrefix
        NoPull = $false; SkipDirty = $false; StashDirty = $false
    ...
    for ($i = 0; $i -lt $argList.Count; $i++) {
        ...
        switch ($normalized)
        {
            ...
            'prefix' { if (-not $valuePart) { if ($i + 1 -lt $argList.Count) { $valuePart = $argList[++$i] } else { $result.Invalid += $raw; break } }; $result.Prefix = $valuePart; continue }
...
$Script:ChildFolderPrefix = $parsedArgs.Prefix
