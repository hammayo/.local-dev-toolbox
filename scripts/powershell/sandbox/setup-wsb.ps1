...
# CONFIGURATION
#
# Paths and credentials below can be overridden via environment variables before
# launching the sandbox, or by editing the defaults in this block directly.
#
# Environment variables:
#   SANDBOX_LOG_FILE        - path for the setup log  (default: C:\TEMP\sandbox_setup.log)
#   SANDBOX_DOWNLOADS       - downloads folder        (default: C:\TEMP\Downloads)
#   CHOCO_SOURCE_URL        - NuGet/Chocolatey feed URL
#   CHOCO_SOURCE_USER       - feed username
#   CHOCO_SOURCE_PASSWORD   - feed password  *** do NOT commit real credentials ***
#   CHOCO_SOURCE_NAME       - feed source name        (default: choco-dev)
#   NODE_VERSION            - Node.js version to install
#   ANGULAR_VERSION         - Angular CLI version to install
#   FORCOURT_VERSION        - Forcourt package version

# Resolve all configurable values before building the config hashtable.
$_logFile        = if ($env:SANDBOX_LOG_FILE)        { $env:SANDBOX_LOG_FILE }        else { "C:\TEMP\sandbox_setup.log" }
$_downloads      = if ($env:SANDBOX_DOWNLOADS)       { $env:SANDBOX_DOWNLOADS }       else { "C:\TEMP\Downloads" }
$_chocoName      = if ($env:CHOCO_SOURCE_NAME)       { $env:CHOCO_SOURCE_NAME }       else { "choco-dev" }
$_chocoUrl       = if ($env:CHOCO_SOURCE_URL)        { $env:CHOCO_SOURCE_URL }        else { "" }
$_chocoUser      = if ($env:CHOCO_SOURCE_USER)       { $env:CHOCO_SOURCE_USER }       else { "" }
$_chocoPassword  = if ($env:CHOCO_SOURCE_PASSWORD)   { $env:CHOCO_SOURCE_PASSWORD }   else { "" }
$_nodeVersion    = if ($env:NODE_VERSION)            { $env:NODE_VERSION }            else { "14.21.3" }
$_angularVersion = if ($env:ANGULAR_VERSION)         { $env:ANGULAR_VERSION }         else { "11.0.6" }
$_forcourtVersion= if ($env:FORCOURT_VERSION)        { $env:FORCOURT_VERSION }        else { "2.6.1.1" }

$Script:Config = @{
    # Paths
    LogFile          = $_logFile
    DownloadsFolder  = $_downloads
    TempFolder       = $env:TEMP
    ...
    # Credentials must be supplied via environment variables — never commit plaintext passwords.
    ChocoSource = @{
        Name     = $_chocoName
        Url      = $_chocoUrl
        User     = $_chocoUser
        Password = $_chocoPassword
        Priority = 1
    }

    # Version requirements
    NodeVersion      = $_nodeVersion
    AngularVersion   = $_angularVersion
    ForcourtVersion  = $_forcourtVersion
}
