...
# -------------------------------------------------------------------------
# Defaults — override via --backup-path / --install-root / --default-user arguments
# or WSL_BACKUP_PATH / WSL_INSTALL_ROOT / WSL_DEFAULT_USER environment variables.
$Script:DefaultBackupPath  = if ($env:WSL_BACKUP_PATH)   { $env:WSL_BACKUP_PATH }   else { 'C:\Backups\WSL' }
$Script:DefaultInstallRoot = if ($env:WSL_INSTALL_ROOT)  { $env:WSL_INSTALL_ROOT }  else { 'D:\WSL' }
# Auto-detect the current Windows username; fall back to env var or explicit arg.
$Script:DefaultUser        = if ($env:WSL_DEFAULT_USER)  { $env:WSL_DEFAULT_USER }
